...
export async function updateBill(db, { companyId, userId }, billId, payload) {
  const existing = await db
    .prepare('SELECT * FROM bills WHERE id = ? AND company_id = ? AND deleted_at IS NULL')
  ...
  if (existing.status !== 'draft') {
    throw new APIError(
      'Only draft documents can be edited. Once a document is confirmed or paid it is locked to keep an accurate financial record — issue a Credit/Debit Note instead.',
      422
    );
  }
  ...
  const statements = [
    db
      .prepare(
        `UPDATE bills SET
          customer_id = ?, bill_date = ?, due_date = ?, place_of_supply = ?,
          subtotal = ?, discount_amount = ?, cgst_amount = ?, sgst_amount = ?, igst_amount = ?,
          round_off = ?, total_amount = ?, balance_amount = ?, notes = ?, terms_and_conditions = ?,
    ...
    db.prepare('DELETE FROM bill_items WHERE bill_id = ?').bind(billId),
  ];
